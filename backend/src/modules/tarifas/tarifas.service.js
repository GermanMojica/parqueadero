...
async function create({ sedeId, tipoVehiculoId, precioHora, fraccionMinutos, vigenteDesde }) {
  ...
  const id = await repo.create({ sedeId, tipoVehiculoId, precioHora, fraccionMinutos, vigenteDesdе: vigenteDesde });
  return { id, message: 'Tarifa creada correctamente' };
}

async function update(id, { precioHora, fraccionMinutos }, sedeId) {
  const [rows] = await pool.execute('SELECT id, activo FROM tarifas WHERE id = ? AND sede_id = ?', [id, sedeId]);
  ...
  await pool.execute(
    'UPDATE tarifas SET precio_hora = ?, fraccion_minutos = ? WHERE id = ? AND sede_id = ?',
    [precioHora, fraccionMinutos, id, sedeId],
  );
...
async function deactivate(id, sedeId) {
  ...
  const [rows] = await pool.execute(
    `SELECT t.tipo_vehiculo_id,
            COUNT(*) AS activas
     FROM tarifas t
     WHERE t.tipo_vehiculo_id = (SELECT tipo_vehiculo_id FROM tarifas WHERE id = ? AND sede_id = ?)
       AND t.activo = 1
       AND t.sede_id = ?
     GROUP BY t.tipo_vehiculo_id`,
    [id, sedeId, sedeId],
  );
  if (rows[0]?.activas <= 1) {
    throw new AppError('No se puede desactivar la única tarifa activa de este tipo de vehículo', 409);
  }
  await repo.deactivate(id, sedeId);
